            ...
            if (len(faces) == 0):
                ...
                try:
                    print('Not face detected ' +
                          str(face['id']) + ' to person - 1')
                    cursor.execute(
                        'UPDATE `faces`.`face` SET `personId`= -1 WHERE  `id`=' + str(face['id']))
                    bd.commit()
                except:
                    bd.rollback()
                    continue
            ...
            for(x, y, w, h) in faces:
                ...
                if (conf > application['face']['recognition']):
                    ...
                    try:
                        # create new person
                        cursor.execute(
                            'INSERT INTO `faces`.`person` (`data`) VALUES (0)')
                        bd.commit()
                    ...
                    try:
                        ...
                        cursor.execute('UPDATE `faces`.`face` SET `personId`= ' +
                                       str(id) + ' WHERE  `id`=' + str(face['id']))
                        bd.commit()
                    except:
                        bd.rollback()
